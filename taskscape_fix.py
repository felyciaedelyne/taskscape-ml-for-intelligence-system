...
# ─────────────────────────────────────────────
# HELPER: draw icon+text centered inside a rect
# FIX #8: ensures icon and text always fit inside button bounds
# ─────────────────────────────────────────────
...
class Game:
    ...
    def _draw_login(self):
        ...
        # ── FIX #3: Start button — properly sized, icon+text centered ──
    ...
    # ═══════════════════════════════════════════
    # DRAW: LEVEL CONFIRM
    # FIX #6: nyawa bonus row alignment fixed
    # ═══════════════════════════════════════════
    def _draw_level_confirm(self):
        ...
        # Row helper — label LEFT, value RIGHT, consistent column widths
        col_label_x = bx + 30
        col_val_x   = bx + box_w - 30   # right-align values from here
        ...
        # ── FIX #6: Nyawa bonus — heart icon inline with value ──
        if self._confirm_life_gained:
            lbl_s = self.fmd.render("Nyawa Bonus", True, C_MUTED)
            val_s = self.fmd.render("+1", True, (255,120,150))
            self.screen.blit(lbl_s, (col_label_x, y))
            # draw +1 text right-aligned
            self.screen.blit(val_s, val_s.get_rect(right=col_val_x, top=y))
            # draw heart icon immediately to the left of the +1 text
            try:
        ...
        # ── FIX #8: Buttons — icon+text always inside box ──
        btn_w,btn_h=220,48
    ...
    # ═══════════════════════════════════════════
    # DRAW: MAZE
    # FIX #2: timer label RIGHT-aligned, bar stays LEFT-aligned
    # ═══════════════════════════════════════════
    def _draw_maze(self):
        ...
        # ── FIX #2: Timer bar LEFT-aligned, "Maze berubah" label RIGHT-aligned ──
        if self.maze_timer_max>0 and self.dots and not self.paused:
            ...
            sw_w=190; sw_h=42
            sw_x=self.MAZE_X          # LEFT edge of maze
            ...
            # "Maze berubah" label RIGHT-aligned to right edge of timer bar
    ...
    # ═══════════════════════════════════════════
    # DRAW: PANEL
    # FIX #5: no "X/9" text — hearts only for lives
    # ═══════════════════════════════════════════
    def _draw_panel(self):
        ...
        # Stats — FIX #5: NYAWA row shows hearts only, no "X/9" number
        stats=[
            ("SCORE", f"{self.score:,}", C_GOLD),
            ("LEVEL", d["label"], col),
            ("NYAWA", None, C_RED),   # None = hearts-only row
    ...
    # ═══════════════════════════════════════════
    # DRAW: GAME OVER
    # FIX #7: "Main Lagi" button centered
    # ═══════════════════════════════════════════
    def _draw_game_over_overlay(self):
        ...
        # ── FIX #7: retry button truly centered, icon inside ──
        btn_w, btn_h = 320, 72
        btn_x = cx - btn_w // 2
        btn_y = cy + 120
    ...
    # ═══════════════════════════════════════════
    # DRAW: PAUSED OVERLAY
    # FIX #1: smaller pause icon, hearts don't overlap text
    # ═══════════════════════════════════════════
    def _draw_paused_overlay(self):
        ...
        # ── FIX #1: smaller pause icon ──
        try:
            pause_img = pygame.image.load(PAUSE_ICON).convert_alpha()
            pause_img = pygame.transform.smoothscale(pause_img, (42, 42))  # was 64
        ...
        d=self._diff()
        # ── FIX #1: score/level text, then hearts on separate line below ──
        ...
        # Hearts on their own line, centered, below the info text
        try:
            heart_img = pygame.image.load(HEART_ICON).convert_alpha()
